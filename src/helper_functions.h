...
inline double multi_variate_gaussian(double x_pos,
                                     double y_pos,
                                     double x_landmark,
                                     double y_landmark,
                                     double sig_x,
  ...
  double exponent = pow(x_pos - x_landmark, 2) / (2 * sig_x * sig_x) + pow(y_pos - y_landmark, 2) / (2 * sig_y * sig_y);
  
  return gauss_norm * exp(-exponent);
}

inline LandmarkObs transform_to_map_coord(double x_pos,
                                          double y_pos,
                                          double heading,
                                          double x_obs,
                                          double y_obs) {
